...
```
deno run --allow-net --allow-read --allow-write --allow-env https://deno.land/x/deno_proxy@v1.1.1/mod.ts
```
...
```
deno run --allow-read --allow-write https://deno.land/x/deno_proxy@v1.1.1/cli/mod.ts --baseUrl http://localhost:8000 --oldPath example/import_map.json --newPath example/import_map_proxy.json
```
