...
```bash
deno run --allow-net --allow-read --allow-write --allow-env https://deno.land/x/deno_proxy@v1.3.11/mod.ts
```
...
```powershell
deno run --allow-read --allow-write https://deno.land/x/deno_proxy@v1.3.11/cli/mod.ts --baseUrl http://localhost:8000 --oldPath example/deps.ts --newPath example/deps_proxy.ts
```
...
```bash
deno run --allow-read --allow-write https://deno.land/x/deno_proxy@v1.3.11/cli/mod.ts --baseUrl http://localhost:8000 --oldPath example/import_map.json --newPath example/import_map_proxy.json
```
